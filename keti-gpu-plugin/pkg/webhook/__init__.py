...
ANNOTATION_PARTITION = os.environ.get('ANNOTATION_PARTITION', 'keti.io/partition')
...
class GPUWebhook:
    ...
    def _get_partition_id(self, node_name: str, partition_name: str) -> Optional[str]:
        """
        PPO Agent에게 파티션 이름(A, B, C)에 대응하는 MPS 파티션 ID 조회

        Args:
            node_name: 노드 이름
            partition_name: 파티션 이름 (A, B, C)

        Returns:
            CUDA_MPS_SM_PARTITION에 넣을 파티션 ID 문자열 또는 None
        """
        node_ip = self._get_node_ip(node_name)
        if not node_ip:
            logger.warning(f"Could not resolve IP for node {node_name}, skipping partition lookup")
            return None

        try:
            url = f"http://{node_ip}:{PPO_AGENT_PORT}/partition"
            payload = {"partition_name": partition_name}
            logger.info(f"Querying partition '{partition_name}' from {url}")

            response = requests.post(url, json=payload, timeout=PPO_AGENT_TIMEOUT)

            if response.status_code == 200:
                data = response.json()
                partition_id = data.get('partition_id')
                if partition_id:
                    logger.info(f"Partition '{partition_name}' -> {partition_id}")
                    return partition_id
                else:
                    logger.warning(f"No partition_id in response: {data}")
                    return None
            else:
                logger.warning(f"Partition lookup returned {response.status_code}: {response.text}")
                return None

        except Exception as e:
            logger.error(f"Partition lookup failed: {e}")
            return None
    ...
    def _generate_patches(self, pod: dict) -> list:
        ...
        # 4. 파티션 어노테이션 처리 (MPS Static Partition)
        partition_name = annotations.get(ANNOTATION_PARTITION, '')
        partition_id = None
        if partition_name and node_name:
            partition_id = self._get_partition_id(node_name, partition_name)
            if partition_id:
                logger.info(f"MPS Partition: {partition_name} -> {partition_id}")
            else:
                logger.warning(f"Failed to resolve partition '{partition_name}', proceeding without MPS partition")

        logger.info(f"GPU injection: cores={cores_percent}%, mem={gpu_mem}, partition={partition_name or 'none'}")
        ...
        for i, container in enumerate(containers):
            ...
            env_patches = self._generate_env_patches(i, container, cores_percent, gpu_mem, partition_id)
            patches.extend(env_patches)

            # Add volume mounts
            mount_patches = self._generate_mount_patches(i, container, partition_id)
            patches.extend(mount_patches)

        # Add volume for vai_accelerator.so (+ MPS pipe if partition)
        if patches:
            volume_patches = self._generate_volume_patches(pod, partition_id)
            patches.extend(volume_patches)

        return patches

    def _generate_env_patches(self, container_idx: int, container: dict, cores_percent: str, gpu_mem: str = '', partition_id: str = None) -> list:
        ...
        # MPS Static Partition 환경변수 추가
        if partition_id:
            env_vars.append({"name": "CUDA_MPS_SM_PARTITION", "value": partition_id})
            env_vars.append({"name": "CUDA_MPS_PIPE_DIRECTORY", "value": "/tmp/nvidia-mps"})

            # 파티션별 libvai shared state 분리
            # 같은 파티션의 Pod끼리만 토큰 풀을 공유하도록
            partition_suffix = partition_id.split('/')[-1][:8] if '/' in partition_id else 'default'
            env_vars.append({
                "name": "VAI_SHM_PATH",
                "value": f"/var/lib/keti/.vai_shared_state_{partition_suffix}"
            })
    ...
    def _generate_mount_patches(self, container_idx: int, container: dict, partition_id: str = None) -> list:
        ...
        # MPS pipe directory mount (파티션 사용 시)
        if partition_id:
            mps_mount_exists = any(
                m.get('mountPath') == '/tmp/nvidia-mps'
                for m in existing_mounts
            )
            if not mps_mount_exists:
                patches.append({
                    "op": "add",
                    "path": f"/spec/containers/{container_idx}/volumeMounts/-",
                    "value": {
    ...
    def _generate_volume_patches(self, pod: dict, partition_id: str = None) -> list:
        ...
        # MPS pipe volume (파티션 사용 시)
        if partition_id:
            mps_volume_exists = any(
                v.get('name') == 'mps-pipe'
                for v in existing_volumes
            )
            if not mps_volume_exists:
                patches.append({
                    "op": "add",
                    "path": "/spec/volumes/-",
                    "value": {
