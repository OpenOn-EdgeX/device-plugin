...
#define VAI_SHM_PATH "/var/lib/keti/.vai_shared_state"
...
static bool init_shared_memory() {
    ...
    vai::shm_fd = open(VAI_SHM_PATH, O_RDWR, 0666);
    bool created = false;

    if (vai::shm_fd < 0) {
        // Create new shared state file
        vai::shm_fd = open(VAI_SHM_PATH, O_CREAT | O_RDWR, 0666);
        if (vai::shm_fd < 0) {
    ...
    if (vai::shared_state == MAP_FAILED) {
        ...
        close(vai::shm_fd);
        if (created) unlink(VAI_SHM_PATH);
        vai::shm_fd = -1;
...
static void cleanup_shared_memory() {
    ...
    if (remaining <= 0) {
        unlink(VAI_SHM_PATH);
        fprintf(stderr, "[vai] Removed shared state file (last process)\n");
    }
}
